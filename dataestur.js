...
export const getSummaryData = async (countries = []) => {
  try {
    const baseUrl = 'http://192.168.1.102:3001/municipalities';
...
export const getDataOfMunicipality = async (municipality) => {
  try {
    const baseUrl = 'http://192.168.1.102:3001/municipality';
...
export const listOriginCountries = async () => {
  try {
    const resp = await fetch('http://192.168.1.102:3001/originCountries');
    if (!resp.ok) throw new Error(`Error en la solicitud: ${resp.status}`);
    const data = await resp.json();
    return data;
  } catch (error) {
